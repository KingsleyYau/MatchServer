...
int DBSpool::ExecuteSQL(const string& strSQL, MYSQL_RES** res, short& shIdentifier, int& iRelt)
{
    ...
    if (i >= 0){
        ...
        int iRet = m_pDBConnection[i].ExecuteSQL(strSQL, res, iRelt);
        int iSize = 0;
...
int DBSpool::ExecuteSQL(const char* pcSQL, MYSQL_RES** res, short& shIdentifier, int& iRelt)
{
    string strSQL = pcSQL;
    return ExecuteSQL(strSQL, res, shIdentifier, iRelt);
}
...
int DBSpool::ExecuteSQL(const string& strSQL, RESDataList* res, int &iRelt)
{
    MYSQL_RES* SQLRes = NULL;
    short shIdt = 0;
    int iRet = ExecuteSQL(strSQL, &SQLRes, shIdt, iRelt);
...
int DBSpool::ExecuteSQL(const char* pcSQL, RESDataList* res, int& iRelt)
{
    string strSQL = pcSQL;
...
int DBConnection::ExecuteSQL(const string& strSQL, MYSQL_RES** res, int& iRelt)
{
    if (m_pSQLConn == NULL){
        return SQL_TYPE_UNKNOW;
    }
...
int DBConnection::ExecuteSQL(const char* pcSQL, MYSQL_RES** res, int& iRelt)
{
    string strSQL = pcSQL;
    return ExecuteSQL(strSQL, res, iRelt);
}
